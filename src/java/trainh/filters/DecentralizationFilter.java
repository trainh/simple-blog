...
public class DecentralizationFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        Properties decentralization
                = (Properties) context.getAttribute("DECENTRALIZATION_LIST");
        String roleString = (String) decentralization.getProperty(resource);
        try {
            HttpSession session = httpRequest.getSession();
            if (session == null || session.getAttribute("USER") == null || roleString == null) {
                chain.doFilter(request, response);
            } else {
                int role = Integer.parseInt(roleString);
                Tbl_Account_DTO dto = (Tbl_Account_DTO) session.getAttribute("USER");
                if (role == 3) {
                    chain.doFilter(request, response);
                } else {
                    if (dto.getRole() != role) {
                        if (role == 1) {
                            //user login
                            request.setAttribute("ERROR_STRING", "User don't use this function!!!");
                            RequestDispatcher rd = request.getRequestDispatcher("invalidPage");
                            rd.forward(request, response);
                        } else {
                            //admin login
                            request.setAttribute("ERROR_STRING", "Admin don't ues this function!!!");
                            RequestDispatcher rd = request.getRequestDispatcher("invalidPage");
                            rd.forward(request, response);
                        }
                    } else {
